function [ ] = glylinkage(file,S,flag)
...
rn		= [dir,name,'.',num2str(S),'.rama.eps'];	% Linkage Plot
...
if flag
	...
	fn1		= [dir,name,'.',num2str(S),'.phi.eps'];		% Linkage plots: phi\psi
	fn2		= [dir,name,'.',num2str(S),'.omega.eps'];	% Linkage plots: omega\psi
	%% Visualization
	figure(1)                          
	h12  = subplot(2,2,2,'Position',[0.3,0.75,0.484,0.1]);
	nx  = hist(x,xbin);
	...
	print(rn,'-depsc2');
	%% Data Collection
	denh	= hist(d,dbin);	denh1 = hist(d1,dbin);	denh2 = hist(d2,dbin);
	data2	= [xbin' nx'/L ybin' ny'/L wbin' nw'/L];
	data3	= [dbin' denh1'/L denh2'/L denh'/L];
	dlmwrite(lh,data2,'delimiter','\t','newline','Unix');
	dlmwrite(dh,data3,'delimiter','\t','newline','Unix');
end %if
end %function
