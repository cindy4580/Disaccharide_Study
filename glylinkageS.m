...
if flag
	...
	nx  = hist(x,xbin);
	bar(xbin,nx/L,'FaceColor',[0 .5 .5],'EdgeColor',[.5 .5 .5],'barwidth',0.9);
	xlim([xmin xmax]);
	ylabel('Frequency','FontSize',8);	set(gca,'fontsize',7);	hold on;

	h13  = subplot(2,2,3,'Position',[0.16,0.1,0.08,0.6]);
	ny  = hist(y,ybin);
	bar(ybin,ny/L,'FaceColor',[0 .5 .5],'EdgeColor',[.5 .5 .5],'barwidth',0.9);
	xlim([ymin,ymax]);	view(h13,270,90);	
	...
	fastscatter(x,y,d);	colorbar;	xlim([xmin xmax]); ylim([ymin ymax]);
	hold on;
	for i = 1 : size(dot,1)
	...
	w		= M(1:L, 4);
	wmax	= 360;    wmin  =    0;
	wbin	= (wmin + 5) :10: (wmax - 5);
	d1		= M(1:L,W - 3);	d2	= M(1:L,W - 2);
	fn1		= [diro,name,'.',num2str(seed),'.phi.eps'];			   	% phi\psi
	fn2		= [diro,name,'.',num2str(seed),'.omega.eps'];			% omega\psi
	%% Visualization
	figure(1)                          
	h12 	= subplot(2,2,2,'Position',[.3, .75, .484, .1]);
	nx  	= hist(x,xbin);
	bar(xbin,nx/L,'FaceColor',[0 .5 .5],'EdgeColor',[.5 .5 .5],'barwidth',.9);
	xlim([xmin xmax]);	ylabel('Frequency','FontSize',8);
	set(gca,'fontsize',7);	hold on;

	h13 	= subplot(2,2,3,'Position',[.16, .1, .08, .6]);
	ny  	= hist(y,ybin);
	bar(ybin,ny/L,'FaceColor',[0 .5 .5],'EdgeColor',[.5 .5 .5],'barwidth',.9);
	xlim([ymin ymax]);	view(h13,270,90);	
	set(gca,'xticklabel',[],'fontsize',7);
	ylabel('Frequency','FontSize',8);	xlabel('\psi(degree)','FontSize',12);
	hold on;

	h14  	= subplot(2,2,4,'Position',[.3, .1, .6, .6]);
	...
	hold off;	print(fn1,'-depsc2'); clear d1;
	figure(2)
	h22  = subplot(2,2,2,'Position',[.3, .75, .484, .1]);
	nw  = hist(w,wbin);
	bar(wbin,nw/L,'FaceColor',[0 .5 .5],'EdgeColor',[.5 .5 .5],'barwidth',.9);
	xlim([wmin wmax]);	ylabel('Frequency','FontSize',8);
	set(gca,'fontsize',7);	hold on;

	h23  = subplot(2,2,3,'Position',[.16, .1, .08, .6]);
	ny  = hist(y,ybin);
	bar(ybin,ny/L,'FaceColor',[0 .5 .5],'EdgeColor',[.5 .5 .5],'barwidth',.9);
	xlim([ymin ymax]);	view(h23,270,90);	
	set(gca,'xticklabel',[],'fontsize',7);
	ylabel('Frequency','FontSize',8);	xlabel('\psi(degree)','FontSize',12);	
	hold on;

	h24  = subplot(2,2,4,'Position',[.3, .1, .6, .6]);
	fastscatter(w,y,d2);	colorbar;	xlim([wmin wmax]);	ylim([ymin ymax]);
	hold on;
	for i = 1 : size(dot,1)
	...
	hold off;	print(fn2,'-depsc2'); clear d2;
%	figure(3)
