function [ ] = enermini3(file,S,stepsize,sim,flag,pos)
% energy matrix  --> Positions of local minima in the trisaccharide context
% 
% INPUT VARIABLES:
...
%       pos      - reducing end/ non-reducing end : r/n
% OUTPUT:
...
name	= char(str(1));
dir		= strcat('~/Data/trimer/',name,'/energy/');
M		= load(strcat(dir,file));
dim		= size(M,2);
xmax	= 360;	xmin = 0;
ymax	= 360;	ymin = 0;
zmax	= 360;	zmin = 0;
mini	= [dir,num2str(S),'s.',num2str(stepsize),'.',sim,'.',pos,'.mini.mat'];
%% Computation
if flag 
