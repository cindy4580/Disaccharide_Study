function [hImage, hText, hXText] = heatmap_CD(mat, xlab, ylab, textmat, varargin)
...
elseif ischar(p.Colormap) || isa(p.Colormap,'function_handle')
    ...
    if strcmp(p.Colormap, 'money')
        if isempty(p.MaxColorValue) && isempty(p.MinColorValue)
            p.Colormap = money(mat, p.ColorLevels);
        elseif (~isempty(p.MaxColorValue)) && (~isempty(p.MinColorValue))
            p.Colormap = money(mat,p.ColorLevels,p.MinColorValue,p.MaxColorValue);
        end
    else
        p.Colormap = feval(p.Colormap,p.ColorLevels);                      % feval: built-in
    end
...
function cmap = money(data, clevels,varargin)
% Function to make the heatmap have the blue, white and red effect
% Handle missing inputs
if nargin < 1, error('Heatmap "Money" requires at least one input argument'); end
if nargin < 2, clevels = 64; end
if nargin == 2
    n = min(data(:));
    x = max(data(:));
end
if nargin == 4
    n = varargin{1};
    x = varargin{2};
    if n > x, error('Need Mincolor in front of Maxcolor'); end
end
if x == n, x = n+1; end
zeroInd = round(-n/(x-n)*(clevels-1)+1);
if zeroInd <= 1 % Just blue
...
elseif zeroInd >= clevels, % Just red
    ...
    b = interp1([1 zeroInd clevels], [0 1 1], 1:clevels); 
    g = interp1([1 zeroInd clevels], [0 1 0], 1:clevels);
