...
%[XMAX,IMAX,XMIN,IMIN] = EXTREMA(X) returns extrema points of the vector X ignoring NaN's, where
...
%	In mathematics, maxima and minima(extrema), are points in the domain of a function at which 
%	the function takes a largest value (maximum) or smallest value (minimum), 
%	either within a given neighbourhood (local extrema) or 
%	on the function domain in its entirety(global extrema).
%
%Example:
%	x = 2*pi*linspace(-1,1);
...
% 2014-08    : Modified by Xindi(cindy) Li for circular data analysis
...
% Vector input?
Nt = numel(x);
if Nt ~= length(x)                          % array size w/o buffer
...
% Creat buffer for each boundary
if size(x,2) == 1
	x = [ x(Nt); x; x(1); ];
else
	x = [ x(Nt) x x(1) ];
...
% Delete positions in buffer and remove duplicates
xmax = unique(xmax,'stable');
imax = unique(imax,'stable');
xmin = unique(xmin,'stable');
imin = unique(imin,'stable');

imax = imax(find(imax ~=1 & imax <= Nt + 1)) - 1;
imin = imin(find(imin ~=1 & imin <= Nt + 1)) - 1;
x = x(2:Nt + 1);
xmax = x(imax);
xmin = x(imin);

% Carlos Adrián Vargas Aguilera. nubeobscura@hotmail.com
% Xindi(Cindy) Li  cindyleehere@hotmail.com
