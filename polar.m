...
if ~hold_state
	...
	for i = (rmin + rinc) : rinc : rmax
		...
		text((i + rinc / 20) * c82, (i + rinc / 20) * s82, ...
                ...
                'HandleVisibility', 'off', 'Parent', cax,'FontSize',7);
	end % for
	...
	for i = 1 : length(th)
		text(rt * cst(i), rt * snt(i), int2str(i * 30),...
                ...
                'HandleVisibility', 'off', 'Parent', cax,'FontSize',5);
		if i == length(th)
			loc = int2str(0);
		else
...
if strcmp(line_style, 'auto')
	q = plot(xx, yy, 'Parent', cax,'color',[0.2 0.6 1.0]);
else
	q = plot(xx, yy, line_style, 'Parent', cax,'color',[0.2 0.6 1.0],'linewidth',1); % Histogram bar linewidth
end % if
if nargout == 1
	hpol = q;
end
