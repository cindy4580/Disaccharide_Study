function [ N, total ] = uncertainty(file,r,S,num,xpos,ypos,flag)
% filtered subsets from MC simulation under density threshold;
...
mini   = [dir,name,'.',num2str(r),'.',num2str(S),'.uncertainty.eps'];
mini_b = [dir,name,'.',num2str(r),'.',num2str(S),'.uncertainty.b.eps'];
...
if flag
	...
	switch S
		case 10
			for i = 1 : num
			load(strcat(dir,'/energy/10s.',num2str(r),'.',num2str(i),'_10','.mini.mat'));
			data10((data10(:,6) > 4.11),:) = [ ];
			temp = [temp; data10];
			end % for i
		case 20
			for i = 1 : num
			load(strcat(dir,'/energy/20s.',num2str(r),'.',num2str(i),'_20','.mini.mat'));
			data20((data20(:,6) > 4.53),:) = [ ];
			temp = [temp; data20];
			end % for i
		case 30
			for i = 1 : num
			load(strcat(dir,'/energy/30s.',num2str(r),'.',num2str(i),'_30','.mini.mat'));
	...
	switch S
		case 10
		scatter(x,y,0.5,'markerfacecolor',[0.8 0.8 0.8],'markeredgecolor',[0.8 0.8 0.8]);	
		box on;	hold on;
		case 20
		scatter(x,y,0.5,'markerfacecolor',[0.7 0.7 0.7],'markeredgecolor',[0.7 0.7 0.7]); 
		box on; hold on;
		case 30	
		scatter(x,y,0.5,'markerfacecolor',[0.6 0.6 0.6],'markeredgecolor',[0.6 0.6 0.6]);      
		box on; hold on;
		case 50	
		scatter(x,y,0.5,'markerfacecolor',[0.45 0.45 0.45],'markeredgecolor',[0.45 0.45 0.45]);
		box on; hold on;
	end %switch
	% hits plot
	for j = 1 : total
	...
	switch S
		case 10
		scatter3(x,y,z,0.5,'markerfacecolor',[0.8 0.8 0.8],'markeredgecolor',[0.8 0.8 0.8]);	
		box on;	axis equal;
		case 20
		scatter3(x,y,z,0.5,'markerfacecolor',[0.7 0.7 0.7],'markeredgecolor',[0.7 0.7 0.7]); 
		box on; axis equal;
		case 30	
		scatter3(x,y,z,0.5,'markerfacecolor',[0.6 0.6 0.6],'markeredgecolor',[0.6 0.6 0.6]);      
		box on;	axis equal;
		case 50	
		scatter3(x,y,z,0.5,'markerfacecolor',[0.45 0.45 0.45],'markeredgecolor',[0.45 0.45 0.45]);
		box on;	axis equal;
	end %switch
	...
	for j = 1 : total
        if N(j,4) == 1 || N(j,4) == 2 
            plotcube([r r r],[N(j,1),N(j,2),N(j,3)],1,[1 1 0]);
			daspect([1,1,1]);	box on;	
		elseif N(j,4) >= 5 && N(j,4) <= 7
            plotcube([r r r],[N(j,1),N(j,2),N(j,3)],1,[0 0 1]);
        	daspect([1,1,1]);	box on;
        elseif N(j,4) >= 8
            plotcube([r r r],[N(j,1),N(j,2),N(j,3)],1,[0 0 0]);
			daspect([1,1,1]);	box on; 
        else
            plotcube([r r r],[N(j,1),N(j,2),N(j,3)],1,[1 0 0]);
			daspect([1,1,1]);	box on; 
        end %if
	end % for
	xlim([0 360]);	ylim([0 360]);	zlim([0 360]);
